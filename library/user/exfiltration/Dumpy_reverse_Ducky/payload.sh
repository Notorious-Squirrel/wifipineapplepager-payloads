...
while true; do
    ...
    if [ "$CURRENT_COUNT" -gt "$INITIAL_COUNT" ]; then
        IS_KBD=$(grep -Ei "Keyboard|HID" /proc/bus/input/devices || true)
        IS_CLASS=$(cat /sys/bus/usb/devices/*/bInterfaceClass 2>/dev/null | grep "03" || true)
        ...
        if [ -z "$DEVICE" ]; then
            LOG red "No USB storage device found."
            RINGTONE warning
            exit 1
        fi
        break
...
if mount -o ro,noatime "$DEVICE" "$MOUNTPOINT" || mount "$DEVICE" "$MOUNTPOINT"; then
    MOUNTED=1
else
    LOG red "Failed to mount $DEVICE"
    RINGTONE warning
    exit 1
fi
...
IFS=$'\n' read -d '' -r -a FILES < "$MANIFEST" || true
COUNT=${#FILES[@]}

if [ "$COUNT" -eq 0 ]; then
    LOG yellow "No files found on device."
    RINGTONE warning
    exit 0
fi
...
while true; do
    ...
    [ "${CHECKED[$INDEX]:-0}" = "1" ] && LOG green "[X] TAGGED" || LOG white "[ ] UNTAGGED"
    ...
    if [ "$KEY" = "UP" ]; then
        ...
        [ "${CHECKED[$INDEX]:-0}" = "1" ] && CHECKED[$INDEX]="0" || CHECKED[$INDEX]="1"
    elif [ "$KEY" = "A" ]; then
        break
    fi
done
...
while IFS= read -r target; do
    ...
    if ! tar -rf "$LOOT_DIR/$ARCHIVE_NAME" "${target#$MOUNTPOINT/}" 2>>"$LOOT_DIR/tar_errors.log"; then
        LOG red "Failed to add: $target"
    fi
done < "$SELECTED"
